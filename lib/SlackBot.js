...
function Slackbot(configuration) {
    ...
    // https://api.slack.com/docs/oauth-scopes
    slack_botkit.createOauthEndpoints = function(webserver, callback, options) {

        options = options || {};
        options.rootRoute = options.rootRoute || '';

        var loginRoute = options.rootRoute + '/login';
        var oauthRoute = options.rootRoute + '/oauth';

        slack_botkit.log('** Serving login URL: http://' +
            slack_botkit.config.hostname + ':' + slack_botkit.config.port + loginRoute);
        ...
        webserver.get(loginRoute, function(req, res) {
            res.redirect(slack_botkit.getAuthorizeURL());
        });

        slack_botkit.log('** Serving oauth return endpoint: http://' +
            slack_botkit.config.hostname + ':' + slack_botkit.config.port + oauthRoute);

        webserver.get(oauthRoute, function(req, res) {
